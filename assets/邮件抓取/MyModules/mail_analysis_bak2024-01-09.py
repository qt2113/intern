...
class Format:
    ...
    def __init__(self):
        ...
        self.MB_TPL = {
            ...
            'dw_net': '单位净值,计提前单位净值,资产份额净值(元),实际净值,A级单位净值,虚拟净值提取前单位净值,份额净值,期末单位净值,基金份额净值',
            'lj_net': '累计单位净值,资产份额累计净值(元),实际累计净值,资产份额累计净值(元),基金份额累计净值,累计净值,A级单位净值,期末累计净值'
        }
        ...
        try:
            ...
            if result:
                ...
                for index,val in enumerate(result):
                    # 没有单位净值
                    if ('dw_net' in val.keys()) is False and 'lj_net' is val.keys():
                        result[index]['dw_net'] = result[index]['lj_net']

                    # 没有累计净值
                    if ('lj_net' in val.keys()) is False and 'dw_net' is val.keys():
